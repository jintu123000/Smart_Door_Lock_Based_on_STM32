...
void uart3_init(uint32_t __Baud)
{
	...
	USART_Init(USART3,&USART_InitStructure);//将相关参数初始化给串口3
...
void uart1_init(uint32_t __Baud)
{
	...
	USART_InitStructure.USART_Mode                  = USART_Mode_Rx | USART_Mode_Tx;//将串口配置为收发模式
	USART_InitStructure.USART_HardwareFlowControl   = USART_HardwareFlowControl_None; //不提供流控 
	USART_Init(USART1,&USART_InitStructure);//将相关参数初始化给串口3
...
uint8_t g_printf_usart = 3;

void set_printf_usart(PRINTF_USART_t usart)
{
    g_printf_usart = usart;
}

/* retarget the C library printf function to the USART */
int fputc(int ch, FILE *f)
{
	if (g_printf_usart == PRINTF_USART1) 
	{
